...
The entropy map makes the batch effect immediately visible. Dark regions (low entropy) correspond to neighborhoods where cells are surrounded primarily by cells from the same center. Bright regions (high entropy) indicate well-mixed neighborhoods. Large swaths of the UMAP are dark, confirming the presence of batch effects.

**Pre-Harmony per-cell KNN entropy: mean = 1.51, SD = 0.73 (max possible = 2.58)**

# Harmony Batch Correction

Harmony was applied to the 19-dimensional marker space, correcting for center identity. After correction, the KNN graph, clustering, and UMAP were all re-computed on the Harmony-corrected embeddings.
...
After Harmony, centers are much better mixed across the UMAP. 
...
```
The post-Harmony entropy map is brighter. Most cells now have higher-entropy neighborhoods, indicating better mixing.
...
The histogram shows the shift: pre-Harmony (red) has a broad distribution extending to very low entropies, while post-Harmony (green) is concentrated at higher entropies. The dashed line marks the theoretical maximum for perfect mixing.
...
**Yes.** The KNN neighborhood entropy approach provides a intuitive, quantitative, and spatially-resolved view of batch effects. By leveraging Seurat's `FindNeighbors` graph and computing Shannon entropy per cell, we can:

- Visualize where batch effects are concentrated on the UMAP
...
The improvement is real but modest. A mean entropy of 1.83 out of 2.58 means the average cell's neighborhood is still not well-mixed. Whether this level of correction is sufficient depends on the downstream application. There could potentially be further improvement if integration is done across more conditions than just the 6 centers.

## Caveats and open questions

1. **Biological signal preservation was not assessed.** This is the most important limitation. The dataset contains 3 conditions (A: site-stained subaliquot 1, B: site-stained subaliquot 2, P: pre-stained), plus Day 1 and Day 2, nested within the 6 centers. Harmony was told to correct for center, but we did not verify whether additional condition-level biological differences survived the correction. In a production analysis, this would need to be examined before proceeding.
...
3. **UMAP and entropy were computed on different cell sets**: UMAP was computed on a 10,000-cell subset, while the entropy metrics were computed on all 72,000 cells using the full KNN graph. The UMAP visualizations and the entropy summary statistics do not correspond to the same cells, though it suffices for the sake of visualization.
...
Batch effects in multi-center CyTOF data can be detected using KNN neighborhood entropy and partially corrected using Harmony, all within the Seurat ecosystem. The kNN entropy approach provides intuitive, quantitative and visual evidence of batch effects. Harmony reduced batch-dominated neighborhoods and improved cross-center mixing, though the correction was incomplete and biological signal preservation was not verified. A production workflow would need to address both of these gaps and would likely benefit from using `HarmonyMatrix()` directly rather than routing through Seurat's scRNA-seq-oriented pipeline.
